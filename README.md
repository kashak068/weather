# HW10 - Taiwan Weather Forecast (14-Day Extended)
...
1. **時間跨度擴充至未來 14 天 (兩週)**：支援全台 22 縣市與各大分區 14 天滾動預報走勢。
2. **涵蓋全台 22 縣市與各大地理分區**：
   ...
   - **離島**：澎湖縣、金門縣、連江縣
3. **即時 CWA API 連線**：支援個人 API Key 自動擷取氣象署即時預報。
4. **SQLite 資料庫快取 (406 筆資料)**：前端直接透過 SQL 查詢本機 `data.db`。
5. **互動式視覺化儀表板**：提供最高溫 (MaxT) / 最低溫 (MinT) 折線圖、KPI 指標卡與 14 天每日表格。
6. **全台 22 縣市互動溫度地圖 (Folium)**：精準定位，4 階溫度色階圓點標記與點擊 Popup 預報卡。
...
```text
weather/
├── fetch_weather.py     # Gate 1: 呼叫 CWA API 取得全台 22 縣市 14 天原始 JSON 資料
├── parse_weather.py     # Gate 2: 解析 JSON，提取全台 22 縣市 14 天氣溫並產出 CSV (406 筆)
...
├── weather_data.csv     # (產出) 清洗後的 14 天預報 CSV (共 406 筆)
